 ...
 * @LastEditTime: 2023-01-04 16:07:56
 * @Author: liu yang
 */
...
export function useTabs() {
  const updateCurrentTab = (tab) => {
    if (typeof tab === 'string') {
      currentTab.value.meta.title = tab;
      document.title = tab || '七石';
      return;
    }
    if (tab?.name === currentTab.value.name) return;
    currentTab.value = Object.assign(currentTab.value, tab);
    document.title = tab?.meta?.menuName || tab?.meta?.title || '七石';
  };
  ...
  const activeTabs = (data, noJump) => {
    if (data.name === currentTab.value?.name) return;
    document.title = data?.meta?.menuName || data?.meta?.title || '七石';
    if (!noJump) {
  ...
  const removeTabs = (data, noJump) => {
    if (cachesTabsStore.cachesTabs.length <= 1) {
      return console.log('删除失败，至少保留一个菜单项');
    }
    cachesTabsStore.removeCachesTabs(data);
    nextTick(() => {
      if (currentTab.value.name === data.name && !noJump) {
  ...
  const refreshCurrentTab = async () => {
    ...
    const idx = cachesTabsStore.cachesTabs.findIndex((v) => v.name === item.name);
  ...
  const rightMenusItemClick = (rightMenusItem) => {
    ...
    let idx = cachesTabsStore.cachesTabs.findIndex((v) => v.name === item.name);
