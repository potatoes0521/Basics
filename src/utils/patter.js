 ...
 * @LastEditTime: 2022-11-28 18:39:01
 * @Author: liu yang
 */
...
export const passwordReg = /^(?![a-zA-Z]+$)(?![A-Z0-9]+$)(?![A-Z\\W_~@#$^]+$)(?![a-z0-9]+$)(?![a-z\\W_~@#$^]+$)(?![0-9\\W_~@#$^]+$)[a-zA-Z0-9\\W_~@#$^]{6,16}$/;
...
export const formValidateIsEditPassword = (rule, value, callback) => {
  if (!value) {
    callback();
    return;
  }
...
/**
 * 获取url里的协议/端口等
 * @param {Object} origin 参数描述
 * @return void
 */
export function matchURLToObject(origin) {
  const [, http, host, port] = origin.match(matchURL);
  const result = [http || 'http', host || '', (port || '').slice(1)];
